        ...
        if not data.empty:
            for fx in fx_tickers:
                if fx in data['Close']:
                    ...
                    if not val.empty: rates[fx] = float(val.iloc[-1])
    except: pass
    return rates

# --- 核心邏輯：股價對齊 (支持全球標的與自動換匯) ---
def get_yahoo_prices(etf_id, stock_ids, target_date_str):
    ...
    print(f"--- [{etf_id}] 啟動全球價格對齊 ({target_date_str}) ---")
    ...
    for sid in clean_ids:
        ...
        if market_code in MARKET_MAP:
            ...
            suffix = m_cfg["suffix"]
            if market_code == "CH" and not ticker_base.startswith("60"): suffix = ".SZ"
            ticker_map[f"{ticker_base}{suffix}"] = {"id": sid, "fx_key": m_cfg["fx"]}
        else:
        ...
        for yt, info in ticker_map.items():
            ...
            if 'Close' in df:
                ...
                if not series.empty:
                    ...
                    prices[sid] = round(float(series.iloc[-1]) * fx_val, 2)
...
def process_and_save(etf_id, holdings, target_date):
            ...
            if f.endswith('.json'):
                try:
                    with open(os.path.join(folder, f), 'r') as j:
                        for s in json.load(j).get('holdings', []): universe_ids.add(s['id'])
                except: continue
    ...
    print(f"✅ {etf_id} 數據存檔成功: {target_date}.json")

# --- ETF 爬蟲模組：統一系列 (支持自動日期遞補) ---
def run_uni_etf(etf_id, fund_code, target_date):
    print(f"📡 啟動 {etf_id} 採集...")
    dt = datetime.strptime(target_date, "%Y-%m-%d")
    check_dates = [dt, dt + timedelta(days=1)] # 檢查今天與明天，解決官網超前問題
    for current_dt in check_dates:
        m_date = f"{current_dt.year - 1911}/{current_dt.strftime('%m/%d')}"
        save_date = current_dt.strftime("%Y-%m-%d")
        try:
            ...
            h = [{"id": d['DetailCode'].strip(), "name": d['DetailName'], "share": d['Share']} 
                 for item in res_json.get('asset', []) if item.get('AssetCode') == 'ST' for d in item.get('Details', [])]
            if h: process_and_save(etf_id, h, save_date)
        except: pass

def run_00982A(target_date):
    print("📡 啟動 00982A 採集...")
    for fmt in [target_date, target_date.replace("-", "/")]:
        try:
            ...
            if stocks:
                ...
                process_and_save("00982A", h, target_date); return
        except: continue
...
def run_00995A(target_date):
    ...
    try:
        auth_res = session.post("https://www.ctbcinvestments.com.tw/API/home/AuthToken", params={"token": "www.ctbcinvestments.com"}, json={"token": "www.ctbcinvestments.com"}, timeout=15)
        token = auth_res.json().get('Data', {}).get('token', '')
        if not token: return
        iso_date = f"{target_date}T{datetime.utcnow().strftime('%H:%M:%S.%f')[:-3]}Z"
        r = session.post("https://www.ctbcinvestments.com.tw/API/etf/ETFHoldingWeight", params={"token": token}, json={"FID": "E0036", "StartDate": iso_date, "token": token}, timeout=20)
...
if __name__ == "__main__":
    ...
    run_uni_etf("00988A", "61YTW", t_date)
    run_00982A(t_date)
    run_00995A(t_date)
